...
contract DoublyLinkedListAddress {
    ...
    mapping(ptr => address) public values;
    ...
    function addValueAtPosition(uint64 _i, address _value) public virtual {
    ...
    function amendValueAtPosition(uint64 _i, address _value) public virtual {
        if (_i >= list.length) revert InvalidPosition();
    ...
    function push(address _value) public virtual {
        ptr valuePtr = _createPtrForValue(_value);
    ...
    function valueAtPosition(uint64 _i) public virtual view returns (address) {
        if (_i >= list.length) revert InvalidPosition();
        ptr positionPtr = list.at(_i);
